<?
		$where = " matching_yn='ALL' or matching_yn='Y'";

		if($st_date <> ""){
			$where .= " and reg_date >= '$st_date'";
		}
		if($ed_date <> ""){
			if($where <> ""){
		...
		if($st_date1 && $st_date2){
			if($where <> ""){
				//$where .= " and payment_in_day between '$st_date1' and '$st_date2'";
				$where .= " and input_date between '$st_date1' and '$st_date2'";
			}else{
				//$where .= " payment_in_day between '$st_date1' and '$st_date2'";
				$where .= " and input_date between '$st_date1' and '$st_date2'";
			}
		}
