...
//	$rf_payment	=	str_change(",", "", $tmp_chk_pay);	//환불
	...
	switch($payment_state){
		...
		case "RF":
			...
			$soaa_reg_check_rs	=	fn_select_fetch_array("a.chk_pay, a.payment_in_day",
															"soaa_reg_check a left join return_list b on a.reg_no=b.reg_no",
														"a.reg_no='" . $reg_no . "'"
														);
			//soaa_reg_check 에서 입금금액을 받아서 return_list 에 추가함.
			$chk_pay			=	$soaa_reg_check_rs['chk_pay'];
//			$input_payment_date	=	$soaa_reg_check_rs['payment_in_day'];
	...
	if($payment_state == "PC"){
		...
		$rs	=	fn_update("payment_state='RF'", "soaa_reg_check", "reg_no='" . $reg_no . "'");
